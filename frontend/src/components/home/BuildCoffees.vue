            ...
            <div class="flex-grow-1 mb-8">
              <v-list density="compact" class="bg-transparent">
                ...
                <div v-if="orderSummaryDetails.length > 0" class="mt-4">
                  ...
                  <v-list-item v-for="(detail, index) in orderSummaryDetails" :key="index"
                    class="px-0 pl-4 text-caption">
...
const addCustomCoffee = async () => {
  ...
  //evitar array de customizações vazio
   if (!selectedBase.value || !selectedSize.value) {
    showSnackbar('Por favor, selecione pelo menos a base e o tamanho do seu café.', 'warning');
    return;
  }

  // 1. Monta o objeto do café personalizado 
  ...
  // função centralizada do composable
  try {
    const successMessage = await addItemToCart(customCoffee);
    showSnackbar(successMessage || `${customCoffee.name} adicionado com sucesso!`, 'success');
  } catch (error) {
